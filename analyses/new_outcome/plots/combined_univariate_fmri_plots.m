...
fprintf(fid,[' We examined the ability of interictal EEG features to '...
    ...
    'features in descending order by effect size. Fig. 2A shows the effect size of the top %d ranked features. '...
    'Circles filled with asterisks indicate those that significantly discriminated the three lateralities (ANOVA with '...
...
fprintf(fid,['<p>We next compared the set of interictal features that best distinguished '...
    ...
    'we ranked features in descending order by their absolute Cohen''s <i>d</i>. Fig. 2B '...
    ...
    'are harder to distinguish than left-sided SOZs in our dataset. Of note, '...
    'several interictal features were highly correlated (see Fig. S1 and Supplemental Results).</p>'],n_to_plot);
...
fprintf(fid,[' We parcellated brain regions according to the DKT atlas, and we identified'...
    ' regions in the temporal lobe gray matter. '...
    ...
    'using fMRI data. Fig. 2C shows the DKT atlas regions included for fMRI connectivity '...
...
fprintf(fid,[' We defined the fMRI connectivity AI using the same method as for '...
    'interictal EEG data (described in the Methods). We compared the AI between '...
    'patients with left-sided SOZs, right-sided SOZs, and bilateral SOZs (Fig. 2D). '...
    'There was a significant difference in AI between groups '...
    ...
    'correcting for multiple comparisons using Bonferroni''s method (%s). '],...
    stats.tbl{2,3},stats.tbl{3,3},stats.tbl{2,5},get_p_html(stats.p),stats.eta2,...
    get_p_html(stats.lbp));

fprintf(fid,['This result suggests that, similar to the result for interictal EEG data, fMRI temporal lobe connectivity '...
...
fprintf(fid,['Of the %d patients and controls studied in the fMRI analysis, '...
    ...
    ', although the effect size is similar (ANOVA: F(%d,%d) = %1.1f, %s, &#951;<sup>2</sup> = %1.2f). '...
    'Therefore, it is unclear how fMRI connectivity AI differs in patients with TLE who '...
    'do not undergo intracranial recording.</p>'],...
