...
fprintf(fid,[' As described in Table 1, the majority of patients underwent bilateral electrode implantation '...
    ...
    'the first hypothesis was left-sided and the second hypothesis was right-sided).']);

fprintf(fid,[' We examined a random sample of 50 automated spike detections from each patient '...
    '(bipolar montage). The median (IQR) percentage of automatically-detected spikes '...
    'determined to be true spikes was %1.1f%% (%1.1f%%-%1.1f%%) for all patients ('...
    '%1.1f%% (%1.1f%%-%1.1f%%) for HUP and %1.1f%% (%1.1f%%-%1.1f%%) for MUSC).</p>'],...
    median(perc_good(all_pts_sp))*100,prctile(perc_good(all_pts_sp),25)*100,...
    prctile(perc_good(all_pts_sp),75)*100,...
