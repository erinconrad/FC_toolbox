...
nb = 1e3;
% Restrict to spikes model for now
%{
ADD BOOTSTRAPPING EVENTUALLY TO GET CONFIDENCE INTERVALS ON AUCS
%}
assert(isequal(lefts.names,rights.names))

% Performance by hospital
hup = contains(lefts.names,'HUP');
musc = contains(lefts.names,'MP');
[~,~,~,AUCLH] = perfcurve(lefts.class(hup),lefts.scores(hup),lefts.pos_class);
[~,~,~,AUCRH] = perfcurve(rights.class(hup),rights.scores(hup),rights.pos_class);
boot_lh = bootstrap_aucs(lefts.class(hup),lefts.scores(hup),lefts.pos_class,nb);
boot_rh = bootstrap_aucs(rights.class(hup),rights.scores(hup),rights.pos_class,nb);
lh_ci = prctile(boot_lh,[2.5,97.5]);
rh_ci = prctile(boot_rh,[2.5,97.5]);


[~,~,~,AUCLM] = perfcurve(lefts.class(musc),lefts.scores(musc),lefts.pos_class);
[~,~,~,AUCRM] = perfcurve(rights.class(musc),rights.scores(musc),rights.pos_class);
boot_lm = bootstrap_aucs(lefts.class(musc),lefts.scores(musc),lefts.pos_class,nb);
boot_rm = bootstrap_aucs(rights.class(musc),rights.scores(musc),rights.pos_class,nb);
...
[~,~,~,AUCLT] = perfcurve(lefts.class(tle),lefts.scores(tle),lefts.pos_class);
[~,~,~,AUCRT] = perfcurve(rights.class(tle),rights.scores(tle),rights.pos_class);
boot_lt = bootstrap_aucs(lefts.class(tle),lefts.scores(tle),lefts.pos_class,nb);
boot_rt = bootstrap_aucs(rights.class(tle),rights.scores(tle),rights.pos_class,nb);
lt_ci = prctile(boot_lt,[2.5,97.5]);
rt_ci = prctile(boot_rt,[2.5,97.5]);

[~,~,~,AUCLE] = perfcurve(lefts.class(etle),lefts.scores(etle),lefts.pos_class);
[~,~,~,AUCRE] = perfcurve(rights.class(etle),rights.scores(etle),rights.pos_class);
boot_le = bootstrap_aucs(lefts.class(etle),lefts.scores(etle),lefts.pos_class,nb);
boot_re = bootstrap_aucs(rights.class(etle),rights.scores(etle),rights.pos_class,nb);
