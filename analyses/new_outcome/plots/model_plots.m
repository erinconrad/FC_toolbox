...
for ia = 1:length(which_refs) 
    ...
    if ia == 1
        fprintf(fid,['We tested whether interictal features can predict SOZ laterality in unseen patients. '...
            ...
            ' bilateral. We first tested these models using all interictal features. '...
            'The AUCs of the ROC of the left- and right-sided models were %1.2f '...
    ...
    if 1
        ...
        if ia == 1
            fprintf(fid,['For the full feature model, we measured feature importance for '...
                'both the left-sided and right-sided classifiers. The sets of the most important features and corresponding '...
                'feature weights were nearly identical for the two classifiers, which may '...
                'reflect that distinguishing left from right-sided SOZs drives much of the model training (Fig. 3D).</p>']);
        end
    ...
    if ia == 1
        fprintf(fid,['<p>Given the high performance of the easy-to-calculate spike rate-only model, we performed '...
            'additional analyses to probe the accuracy of this model. '...
            'Confusion matrices for the left- and right-sided models at the optimal '...
            'operating points (with a misclassification cost weighted by class imbalance) '...
    ...
    if ia == 1
        fprintf(fid,['We next examined how the '...
            ...
            'segments amounting to several different total durations and reconstructed the models, using ten different '...
    ...
    if ia == 1
        fprintf(fid,['<p>Finally, we tested how the spike-only models performed in an '...
                'external dataset. We trained the models using only spike rate AI '...
