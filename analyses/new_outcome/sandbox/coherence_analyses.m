...
switch dims
    case 3
        mt_lr = nan(npts,2);
        mt_lr(:,1) = squeeze(nanmean(thing_symm_atlas(:,strcmp(lats,'L')&mt,strcmp(lats,'L')&mt),[2 3]));
        mt_lr(:,2) = squeeze(nanmean(thing_symm_atlas(:,strcmp(lats,'R')&mt,strcmp(lats,'R')&mt),[2 3]));

    case 4
        mt_lr = nan(npts,2,size(thing_symm_atlas,4));
        mt_lr(:,1,:) = squeeze(nanmean(thing_symm_atlas(:,strcmp(lats,'L')&mt,strcmp(lats,'L')&mt,:),[2 3]));
        mt_lr(:,2,:) = squeeze(nanmean(thing_symm_atlas(:,strcmp(lats,'R')&mt,strcmp(lats,'R')&mt,:),[2 3]));
