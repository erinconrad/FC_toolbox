function test_simpler_classifier(which_atlas,nb)
...
do_plot = 1;
params.models = {'ana','ana_cov','ana_cov_ns',...
    'ana_cov_spikes','ana_cov_spikes_ns','ana_cov_intra_extra'}; % which models
params.pretty_name = {'Anatomy','Add coverage density','Add connectivity','Add spike rates','All','All intra extra'};
...
%% Get average functional connectivity
ns = cellfun(@(x) nanmean(x,2),fc,'uniformoutput',false); % average rather than sum

%% Get intra-regional and extra-regional average connectivity
% Short range versus long range connections

% For each electrode, find electrodes belonging to the same region
same_regions = cellfun(@(x,y,z) find_same_regions(x,y,z), ...
    atlas_elec_regions,atlas_elec_labels,labels,'uniformoutput',false);

if 0
    turn_nans_gray(same_regions{100})
end

% For each electrode, calculate the average connectivity to those in the
% same region and those outside
[intra,extra] = cellfun(@(x,y) find_intra_extra_connectivity(x,y),...
    fc,same_regions,'uniformoutput',false);
...
%% Define search radius to be the maximum inter-electrode distance
% So all electrodes are counted
sr = cellfun(@(x) define_sr_maxD(x),locs); % a different SR per patient


%% Get info on missing info
missing_stuff = zeros(npts,7);
...
for ip = 1:npts
    ...
    % get density
    density = estimate_coverage_density(curr_loc,curr_sr,curr_labels);
    ...
    if do_norm
        ...
        curr_intra = (curr_intra - nanmean(curr_intra))./nanstd(curr_intra);
        curr_extra = (curr_extra - nanmean(curr_extra))./nanstd(curr_extra);
    end
    ...
    vec_intra = [vec_intra;curr_intra];
    vec_extra = [vec_extra;curr_extra];
    ...
    if all(isnan(curr_intra))
        missing_stuff(ip,6) = 1;
    end
    
    if all(isnan(curr_extra))
        missing_stuff(ip,7) = 1;
    end
    
end

%% Make table
T = table(vec_soz,vec_pt_idx,vec_rate,vec_loc,vec_ns,vec_dens,vec_labels,vec_intra,vec_extra);
...
assert(sum(isnan(T.vec_soz) | isnan(T.vec_pt_idx)|isnan(T.vec_ns) | ...
    isnan(T.vec_dens)) == 0);
T(nan_rows,:) = [];
...
%% Remove those with missing intra or extra
nan_rows = isnan(T.vec_intra) | isnan(T.vec_extra);
T(nan_rows,:) = [];
n_exc_intra_extra = npts - n_exc_spikes - n_exc_locs -length(unique(T.vec_pt_idx));
...
mout.n_exc_intra_extra = n_exc_intra_extra;
...
while 1 % wrap this all in a while loop to try again if wacky errors related to bad selection (occurs in ~1/100-1/200 times)
    ...
    for im = 1:length(models)
        ...
        switch curr_model
            ...
            case 'ana_cov_intra_extra'
                formula = 'vec_soz ~ vec_loc + vec_dens + vec_rate+ vec_intra + vec_extra';
                formula_me = 'vec_soz ~ vec_loc + vec_dens + vec_rate+ vec_intra + vec_extra + (1|vec_pt_idx)';
        end
...
function same_regions = find_same_regions(regions,atlas_labels,fc_labels)
...
%% Reconcile labels
assert(isequal(fc_labels,atlas_labels))

nelecs = length(regions);
same_regions = nan(nelecs,nelecs);

for ie = 1:nelecs
    same_regions(ie,:) = regions == regions(ie);
    
    % say it is not in its own region
    same_regions(ie,ie)=  0;
end

end

function [intra,extra] = find_intra_extra_connectivity(fc,same_regions)

nelecs = size(same_regions,1);
intra = nan(nelecs,1);
extra = nan(nelecs,1);

fc(logical(eye(size(fc)))) = nan;

for ie = 1:nelecs
    intra(ie) = nanmean(fc(ie,logical(same_regions(ie,:))));
    extra(ie) = nanmean(fc(ie,~logical(same_regions(ie,:))));
end
