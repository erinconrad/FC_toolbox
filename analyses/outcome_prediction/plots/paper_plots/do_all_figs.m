...
for ia = 1:2
    ...
    fprintf(fid,[' Across patients, there were on average %1.1f (range %d-%d) regions '...
        ...
        mean(sum(atlas_out.all_bilateral,1)/2),min(sum(atlas_out.all_bilateral,1)/2), max(sum(atlas_out.all_bilateral,1)/2),...
        mean(sum(atlas_out.all_bilateral,1)),sum(sum(atlas_out.all_bilateral,1)>0),size(atlas_out.all_bilateral,2));
    ...
    fprintf(fid,[' The average intra-hemispheric connectivity on the side of the SOZ (median %1.1e) was also lower than that in '...
...
for ia = 1:2
    ...
    for f = 1:5
        ...
        if stats.pval < 0.05
            ...
            fprintf(fid,[' There was no significant difference between the %s coherence to the SOZ (median %1.1e) and that to '...
                ' the region contralateral to the SOZ (median %1.1e) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Fig. 1C).'],...
                freqs{f},stats.medians(1),stats.medians(2),stats.Tpos,get_p_html(stats.pval));
        end
...
for ia = 1:2
    ...
    fprintf(fid,[' We restricted analysis to patients with strictly unilateral localization '...
        'of the SOZ (%d with left-sided onset, and %d with right-sided onset).'],...
        sum(strcmp(soz_lats,'left')),sum(strcmp(soz_lats,'right')));
    ...
    for i = 1:2 % connectivity then spikes
        ...
        if i == 1
            ...
            ttext = 'Connectivity';
            fprintf(fid,[' Our prediction was %1.1f%% accurate (PPV %1.1f%%, NPV %1.1f%%) '...
            ...
            if ia == 1
                fprintf(fid,[' For comparison, a prediction using spike rates '...
                    ...
                    'those with symmetric coverage) was %1.1f%% accurate (PPV %1.1f%%, NPV %1.1f%%) '...
    ...
    fprintf(fid,[' These results indicate that reduced intra-hemispheric connectivity '...
        'can lateralize epilepsy with similar accuracy to using spike rates.</p>']);
    fclose(fid);
end
