...
for i = 1
    ...
    set(gca,'fontsize',15)
    title({'Change in connectivity with single spikes',' (All channels)'})
   ...
   fprintf(fid,[' In an attempt to understand if the negative relationship between spikes and connectivity '...
    ...
    title({'Change in connectivity with single spikes',' (Channels involved in spike)'})
    set(gca,'fontsize',15)
    
    % EDIT THIS TO BE CONNECTIVITY TO SINGLE CHANNEL
    fprintf(fid,[' In order to examine the local effect of spikes, we also compared the pre- and during-spike '...
        'connectivity for a local network including only the electrodes involved in the spike '...
        '(defined to be those electrodes with a spike detected in the 6-second window '...
        'centered around the chosen spike).']);
