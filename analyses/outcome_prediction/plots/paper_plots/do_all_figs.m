...
for ia = 1:2
    ...
    fprintf(fid,['<p>Finally, we used the estimate of the model coeffiencients to '...
        ...
        'electrode decreased by %s%% (bootstrap 95%% CI OR [%s-%s]) for each additional '...
        'normalized connectivity unit (bootstrap %s).'],(pretty_exp_html((1-or)*100)),pretty_exp_html(ci95(1)),...
        pretty_exp_html(ci95(2)),get_p_html(p));
