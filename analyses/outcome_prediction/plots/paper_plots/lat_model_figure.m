...
fprintf(fid,[' We restricted analysis to patients with unilateral seizure onset '...
...
fprintf(fid,[' We constructed a logistic regression classifier to predict seizure onset '...
    ...
    ' The average accuracy of seizure onset zone laterality predictions was %1.1f%%'...
    ' (positive predictive value (PPV) for detecting left-sided seizure onset: %1.1f%%, negative predictive value (NPV): %1.1f%%)(Fig. 4A).'...
    ' Using a model based on the AAL atlas rather than the Brainnetome atlas '...
    'yielded an average accuracy of %1.1f%% (PPV: %1.1f%%, NPV: %1.1f%%) (Fig. 4B).</p>'],...
    lat_info(1).all_accuracy(1)*100,lat_info(1).all_lpv,lat_info(1).all_rpv,...
    lat_info(2).all_accuracy(1)*100,lat_info(1).all_lpv,lat_info(1).all_rpv);
