...
fprintf(fid,['We next measured the correlation between functional connectivity and spike rates. '...
    '%d patients had accurate spike detections (defined by the requirement that >70% of '...
    'randomly plotted spike detections appeared to be real spikes by visual analysis) and were included in this analysis.'...
    ' For each patient, we took the Spearman rank correlation between the vector containing the average spike rate '...
    'for each electrode and the vector containing the average functional connectivity of each electrode.'],sum(good_spikes));
for i = 1
    if i == 1
        thing = spike_fc_corr;
   ...
   fprintf(fid,[' In an attempt to understand if the negative relationship between spikes and connectivity '...
    ...
    fprintf(fid,[' In order to examine the local effect of spikes, we also compared the average pre- and during-spike '...
        ...
        '(Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (%sC). '...
        'This implies that the average connectivity to the spike electrode is reduced during a spike, '...
        'which is the opposite result as for global connectivity. '...
        ...
        'electrodes may be due, in part, to the spikes themselves.</p>'],...
...
for ia = 1:2
    ...
    fprintf(fid,[' We compared the performance of the null model against a model including connectivity '...
        ...
        '(AUC = %1.2f; Table 2 shows statistics of model comparisons). Notably, using a conservative '...
        ...
        mean(model_info(4).all_auc),mean(model_info(5).all_auc),get_p_html(model.all_out.model_p(3,2)));
    ...
    fprintf(fid,['<p>We anticipated that the spatial null model and other models ('...
        ...
        'of each model was higher in the case of stereo-EEG implantations, although no difference was significant (%sB; Table S2).</p>'],...
