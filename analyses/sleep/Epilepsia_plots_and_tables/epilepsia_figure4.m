...
fprintf(fid,['<br>We asked whether patients demonstrated a circadian rhythm '...
    ...
    'demonstrating more seizures in the morning and afternoon (Figure S3A).'],...
    z,get_p_html(pval));
...
fprintf(fid,[' Seizure rates were similar between sleep (median %1.3f seizures/day)'...
    ' and wake (median %1.3f seizures/day) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Figure S3B).'],...
...
fprintf(fid,['<p>We next examined spike rates surrounding seizures. '...
    ...
    'tracked with a postictal increase in sleep classification (Fig. 4A).']);
...
fprintf(fid,[' Across all patients, the median spike rate postictally (median %1.2f spikes/elecs/min)'...
    ...
    '(Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s, effect size r = %1.2f) (Fig. 4B).'...
    ...
    'alternative peri-ictal time windows) (Table S6).'],...
...
fprintf(fid,[' Patients with temporal lobe epilepsy had a greater '...
    ...
    ' %1.1f, %s, effect size r = %1.2f) (Fig. 4C). This was also true when the pre- and post-ictal'...
    ' windows were defined to be 3 hours each (<i>p</i> = 0.017) and when they '...
    'were defined to be 12 hours each (<i>p</i> < 0.001) (Table S6).'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
...
fprintf(fid,[' Patients with mesial temporal epilepsy also had a greater '...
    ...
    ' epilepsy (Figure S2B, Table S1). There was no clear preictal change in spikes (Supplemental Results, Fig. 4A).']);
...
fprintf(fid,[' Finally, we tested if seizure type influenced the pre-to-postictal spike rate change. '...
    'We performed a linear mixed effects model examining %d seizures from %d patients with '...
    ...
    'The estimate for the post-preictal spike rate difference was %1.2f spikes/elecs/min (95%% CI %1.2f-%1.2f, %s), again '...
    'implying that spike rates increase postictally. '...
    'There was no significant effect of seizure type on post-ictal spike rate change (Fig. 4D, Table S7).<p>'],...
...
fprintf(sfid,['Visually, there was no obvious preictal increase in spikes (Fig. 4A).'...
    ...
    '(Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Figure S3C).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));

% Secondary analyses
fprintf(sfid,[' However, spike rates were significantly higher in the late preictal period than'...
    ' in the early preictal period when performing our secondary analysis of 3-hour preictal windows (<i>p</i> = 0.016) '...
    'and 12-hour preictal windows (<i>p</i> = 0.018) (Table S6).']); 
...
fprintf(sfid,[' For the primary analysis of a 6-hour preictal period, the early-to-late'...
    ...
    '(<i>N<sub>temporal</sub></i> = %d, <i>N<sub>extra-temporal</sub></i> = %d) ='...
    ' %1.1f, %s) (Figure S3D). There was also no significant difference in the preictal spike '...
    'rate change between epilepsy localizations when studying the 3-hour '...
    '(<i>p</i> = 0.64) or 12-hour preictal windows (<i>p</i> = 0.36) (Table S6).'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
