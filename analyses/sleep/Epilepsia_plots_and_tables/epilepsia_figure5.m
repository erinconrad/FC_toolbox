...
fprintf(fid,['We tested how accurately spike rates could classify electrodes '...
    'as SOZ versus non-SOZ. We constructed a mixed-effects logistic regression classifier.'...
    ' Each electrode from each patient was a separate observation. The response '...
    'was a binary variable indicating whether the electrode was SOZ or non-SOZ.'...
    ' The fixed effects predictors were the average spike rate in wakefulness, the average spike '...
    ...
    'The patient identifier was a random effect. To calculate statistics on model coefficients, we used bootstrapping (N = 1,000), '...
    'where for each bootstrap sample we randomly selected patients with replacement. '...
    'We excluded patients without seizures '...
    '(N = %d), patients without detected sleep (N = %d), and patients for whom the model '...
    'failed to converge (N = %d), resulting in %d remaining patients. Holding other '...
    'predictors constant, the odds of an electrode being a SOZ increased significantly '...
    ...
    'in the SOZ. However, we found in this case that spikes in the preictal state but not the postictal '...
    'state increased the likelihood of an electrode belonging to the SOZ (Supplemental Results).</p>'], ...
    4,1,1,sum(~isnan(aucs)),(exp(coeff_stats(1,1))-1)*100,exp(coeff_stats(1,2)),exp(coeff_stats(1,3)),get_p_html(coeff_stats(1,4)),...
...
fprintf(sfid,['We repeated the SOZ localization analysis, only including electrode '...
    ...
    'and with an increase in preictal spike rate (%1.1f%%, 95%% CI [%1.2f-%1.2f], %s), '...
    'but not with an increase in spike rate in wakefulness (%1.1f%%, 95%% CI [%1.2f-%1.2f], %s) '...
...
fprintf(fid,['<p>We next assessed how well this model would '...
    ...
    nanmean(aucs,1),nanstd(aucs,[],1),nanmean(nmout_gray.pt_stats(:,5),1),nanstd(nmout_gray.pt_stats(:,5),[],1));
...
fprintf(fid,['<p>To provide an example of real-world classifier performance, we set a'...
    ...
    '%1.1f%%, and a negative predictive value (NPV) of %1.1f%% (Fig 5B). The large discrepancy between '...
    'PPV and NPV reflects the fact that most electrodes do not belong to the SOZ. As a specific example of '...
    'how this would perform in a single patient, if a patient had 110 electrode contacts, 10 of which '...
    'were in the clinician-defined SOZ, of the 10 model-predicted SOZ contacts, on average %1.1f '...
    'of these would belong to the clinician-defined SOZ (and %1.1f would not). And of the 100 contacts '...
    'predicted not to be in the SOZ by the model, %1.1f would actually belong to the clinician-defined SOZ.</p>'],...
    nanmean(threshold_mat), nanstd(threshold_mat),all_acc*100,...
    all_ppv*100,all_npv*100,...
    10*all_ppv, 10-10*all_ppv,100-100*all_npv);
...
fprintf(fid,'<p>Model performance was highly variable across patients (Fig 5C).');
...
fprintf(fid,[' Patients with stereo-EEG implantations had similar '...
    ...
    '(median PPV = %1.2f) to those with grid/strip/depth implantations'...
    ' (median PPV = %1.2f) (Mann-Whitney test: <i>U</i>'...
...
fprintf(fid,[' Patients with temporal lobe epilepsy had higher '...
    'classification positive predictive value '...
    '(median PPV = %1.2f) relative to those with extra-temporal'...
    ' lobe epilepsy (median PPV = %1.2f) (Mann-Whitney test: <i>U</i>'...
...
fprintf(fid,['<p>We next studied what duration of interictal data was needed '...
    ...
    ' Using durations of 1 minute, 5 minutes, 10 minutes, 30 minutes, and 60 minutes, we chose random 1-minute segments '...
    ...
    'We measured the AUC of the model tested on the testing data. This resulted in a single '...
