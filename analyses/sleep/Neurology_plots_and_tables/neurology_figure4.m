...
fprintf(fid,['<br>We asked whether patients demonstrated a circadian rhythm '...
    ...
    'demonstrating more seizures in the morning and afternoon and fewer in the evening (eFigure 2A).'],...
    z,get_p_html(pval));
...
fprintf(fid,[' Seizure rates were not significantly different between sleep (median %1.3f seizures/day)'...
    ' and wake (median %1.3f seizures/day) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (eFigure 2B).'],...
...
fprintf(fid,[' Across all patients, the median spike rate postictally (median %1.2f spikes/elecs/min)'...
    ...
    'alternative peri-ictal time windows) (eTable 1).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));
...
fprintf(fid,[' Patients with temporal lobe epilepsy had a greater '...
    ...
    ' %1.1f, %s) (Fig. 4C). This was also true when the pre- and post-ictal'...
    ' windows were defined to be 3 hours each (<i>p</i> = 0.019) and when they '...
    'were defined to be 12 hours each (<i>p</i> < 0.001) (eTable 1).'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
    nt,ne,U,get_p_html(p));

fprintf(fid,[' Patients with mesial temporal epilepsy also had a greater '...
    'pre-to-postictal increase in spike rates '...
    'than did those with neocortical'...
    ' epilepsy (eFigure 1B, eTable 2).</p>']);
...
% Results text
fprintf(fid,['<p>Visually, there was no obvious preictal increase in spikes (Fig. 4A).'...
    ...
    '(Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (eFigure 2C).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));
...
fprintf(fid,[' However, spike rates were significantly higher in the late preictal period than'...
    ...
    'and 12-hour preictal windows (<i>p</i> = 0.017) (eTable 1).']); 
...
fprintf(fid,[' For the primary analysis of a 6-hour preictal period, the early-to-late'...
    ...
    '(<i>N<sub>temporal</sub></i> = %d, <i>N<sub>extra-temporal</sub></i> = %d) ='...
    ' %1.1f, %s) (eFigure 2D). There was also no significant difference in the preictal spike '...
    'rate change between epilepsy localizations when studying the 3-hour '...
    '(<i>p</i> = 0.64) or 12-hour preictal windows (<i>p</i> = 0.40) (eTable 1).'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
    nt,ne,U,get_p_html(p));
