...
%% Parameters
surround_hours = 12; % main is 6, will also check 3 and 12
...
for p = 1:npts
    ...
    sp_count_bins_all_elecs = nan(size(bins,1),size(spikes,1),2);
    ...
    for s = 1:size(bins,1)
        ...
        curr_pre_bins = curr_bins(pre_bins);
        curr_post_bins = curr_bins(post_bins);
        pre_bins_in_range = curr_pre_bins(curr_pre_bins > 0 & curr_pre_bins <= size(avg_spikes,2));
        post_bins_in_range = curr_post_bins(curr_post_bins > 0 & curr_post_bins <= size(avg_spikes,2));
        ...
        sp_count_bins_all_elecs(s,:,1) = nanmean(spikes(:,(pre_bins_in_range)),2);
        sp_count_bins_all_elecs(s,:,2) = nanmean(spikes(:,(post_bins_in_range)),2);
    ...
    pre_post_ictal_rates{p} = [(squeeze(nanmean(sp_count_bins_all_elecs(s,:,1),1)))',...;
                        (squeeze(nanmean(sp_count_bins_all_elecs(s,:,2),1)))'];
