...
out_folder = [results_folder,'analysis/sleep/neuron/'];
...
fprintf(fid,['We tested how accurately spike rates could classify electrodes '...
    ...
    'state were not associated with the likelihood of an electrode belonging to the SOZ (Supplemental Results).</p>'], ...
...
fprintf(sfid,['We repeated the SOZ localization analysis, only including electrode '...
    ...
    'but not with an increase in spike rate in wakefulness (%1.1f%%, 95%% CI [%1.2f-%1.2f], %s), '...
    'or an increase in postictal spike rate (%1.1f%%, 95%% CI [%1.2f-%1.2f], %s). In '...
    'this analysis, there was a small positive effect of <i>pre</i>-ictal spike rate (%1.1f%%, 95%% CI [%1.2f-%1.2f], %s)</p>'], ...
    perc_in_gray*100,...
...
fprintf(fid,['<p>We next assessed how well this model would '...
    ...
    'an ROC curve representing classifier performance on the leave-one-out test patient was %1.2f (%1.2f) (%sA). '...
    'Results were similar for a model analyzing electrodes in gray matter only '...
    '(mean (SD) AUC =  %1.2f (%1.2f)). '],...
    nanmean(aucs,1),nanstd(aucs,[],1),fig_name,nanmean(nmout_gray.pt_stats(:,5),1),nanstd(nmout_gray.pt_stats(:,5),[],1));
...
fprintf(fid,['To provide an example of real-world classifier performance, we selected patient-specific thresholds, '...
    ...
    '%1.1f%%, and a negative predictive value (NPV) of %1.1f%% (%sB). '],...
    all_acc*100,...
    all_ppv*100,all_npv*100,fig_name);
...
fprintf(fid,'Model performance was highly variable across patients (%sC).',fig_name);
...
fprintf(fid,[' Patients with stereo-EEG implantations had similar '...
    ...
    ' %1.1f, %s) (%sD).'],nanmedian(aucs(stereo)),nanmedian(aucs(~stereo)),...
    nt,ne,U,get_p_html(p),fig_name);
...
fprintf(fid,[' Patients with temporal lobe epilepsy had higher '...
    ...
    ' %1.1f, %s) (%sE).</p>'],nanmedian(aucs(temporal)),nanmedian(aucs(extra)),...
    nt,ne,U,get_p_html(p),fig_name);
...
fprintf(fid,['<p>We next studied what duration of interictal data was needed '...
    ...
    mean_estimate_10,ste_estimate_10,fig_name);
%}
...
print(gcf,[out_folder,'Fig4'],'-depsc')
close all
