...
fprintf(fid,['<p>We asked how spike rates changed with sleep.'...
    ...
    'spike rates demonstrate a circadian rhythm (Figure 2A).']); 
...
fprintf(fid,[' Examination of spike rates by time of day revealed a non-uniform distribution'...
    ...
    ' afternoon, and evening (Figure 2B).'],z,get_p_html(pval)); 
...
% Results text
fprintf(fid,[' Across all patients, spike rates were higher in sleep (median %1.1f spikes/elecs/min)'...
    ' than wake (median %1.1f spikes/elecs/min) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s).'...
    ' Spike rates were higher in sleep for %d out of %d patients (Figure 2C).</p>'],...
...
fprintf(fid,[' The number of independent spike sequences was higher in sleep (median %1.1f spikes/elecs/min)'...
    ' than wake (median %1.1f spikes/elecs/min) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Figure 2D).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));
...
% Results text
fprintf(fid,[' The spike spread was also higher in sleep (median %1.1f spikes/elecs/min)'...
    ' than wake (median %1.1f spikes/elecs/min) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Figure 2E).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));
...
fprintf(fid,[' We measured the consistency in the patterns of spike rates and spike spread '...
    ...
    'in sleep and that in wake. The spike rates and spread patterns were highly consistent between'...
    ' wake and sleep (median &#961 across patients: %1.2f for rate, %1.2f for spread) (Figure 2F).</p>'],...
...
fprintf(fid,['<p>To test a potential mechanism for the increased spike rates in sleep,'...
    ...
    ' than wake (median %1.1f) (Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Figure 2G).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));
...
fprintf(fid,[' Finally, we tested how overall spike rates and sleep-wake differences in spike'...
    ...
    ' with temporal lobe epilepsy (median = %1.1f) and patients with extra-temporal'...
    ' lobe epilepsy (median = %1.1f) (Mann-Whitney test: <i>U</i>'...
    '(<i>N<sub>temporal</sub></i> = %d, <i>N<sub>extra-temporal</sub></i> = %d) ='...
    ' %1.1f, %s) (Figure 2H).'],nanmedian(rate(temporal)),nanmedian(rate(extra)),...
...
fprintf(fid,[' There was also no difference in overall spike rates between patients'...
    ...
    ' lobe epilepsy (median = %1.1f) (Mann-Whitney test: <i>U</i>'...
    '(<i>N<sub>temporal</sub></i> = %d, <i>N<sub>extra-temporal</sub></i> = %d) ='...
    ' %1.1f, %s) (Figure 2I).<p>'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
    nt,ne,U,get_p_html(p));
%{
