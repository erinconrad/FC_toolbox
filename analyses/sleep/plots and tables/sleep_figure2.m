...
fprintf(fid,['<p>We next looked at spike rates surrounding seizures. '...
    ...
    'tracked with a postictal increase in sleep classification (Figs. 3D and 3G).']);
...
fprintf(fid,[' Patients with temporal lobe epilepsy had a greater '...
    ...
    'were defined to be 12 hours each (<i>p</i> < 0.001) (Supplemental Table 1).</p>'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
    nt,ne,U,get_p_html(p));
...
fprintf(fid,['<p>Visually, there was no obvious preictal increase in spikes (Fig. 3D).'...
    ...
    '(Wilcoxon signed-rank test: <i>T<sup>+</sup></i> = %1.1f, %s) (Fig. 3H).'],...
    stats.medians(2),stats.medians(1),stats.Tpos,get_p_html(stats.pval));

% Secondary analyses
fprintf(fid,[' However, spike rates were significantly higher in the late preictal period than'...
    ' in the early preictal period when performing our secondary analysis of 3-hour preictal windows (early median 0.49 spikes/elecs/min, '...
    ...
    '<i>T<sup>+</sup></i> = 1615.0, <i>p</i> = 0.020) (Supplemental Table 1).']); 
...
fprintf(fid,[' For the primary analysis of a 6-hour preictal period, the early-to-late'...
    ...
    '(<i>N<sub>temporal</sub></i> = %d, <i>N<sub>extra-temporal</sub></i> = %d) ='...
    ' %1.1f, %s) (Fig. 3I). There was also no significant difference in the preictal spike '...
    'rate change between epilepsy localizations when studying the 3-hour '...
    '(<i>p</i> = 0.66) or 12-hour preictal windows (<i>p</i> = 0.06) (Supplemental Table 1).</p>'],nanmedian(rate_diff(temporal)),nanmedian(rate_diff(extra)),...
    nt,ne,U,get_p_html(p));
...
annotation('textbox',[0.67 0.23 0.1 0.1],'String','I','fontsize',25,'linestyle','none')
