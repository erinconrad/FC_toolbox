...
% Turn to table
female_str = {'Female: N (%)',fprintf('%d (%1.1f%%)',nfemale,nfemale/npts)};
age_onset_str = {'Age at onset in years: median (range)',fprintf('%1.1f (%1.1f-%1.1f)',...
    median_range_age_onset(1),median_range_age_onset(2),median_range_age_onset(3))};
age_implant_str = {'Age at implant in years: median (range)',fprintf('%1.1f (%1.1f-%1.1f)',...
    median_range_age_implant(1),median_range_age_implant(2),median_range_age_implant(3))};
nelecs_str = {'Number of electrodes: median (range)',fprintf('%1.1f (%1.1f-%1.1f)',...
    median_range_nelecs(1),median_range_nelecs(2),median_range_nelecs(3))};
duration_str = {'Intracranial recording duration in days: median (range)',fprintf('%1.1f (%1.1f-%1.1f)',...
    median_range_duration(1),median_range_duration(2),median_range_duration(3))};
rate_str = {'Spike rate (spikes/elecs/min): median (range)',fprintf('%1.1f (%1.1f-%1.1f)',...
    median_range_rate(1),median_range_rate(2),median_range_rate(3))};
lat_str = {'Seizure laterality',''};
left_str = {'Left: N (%)',fprintf('%d (%1.1f%%)',n_left,n_left/npts)};
right_str = {'Right: N (%)',fprintf('%d (%1.1f%%)',n_right,n_right/npts)};
bilateral_str = {'Bilateral: N (%)',fprintf('%d (%1.1f%%)',n_bilateral,n_bilateral/npts)};
loc_str = {'Seizure localization',''};
temporal_str = {'Temporal: N (%)',fprintf('%d (%1.1f%%)',n_temporal,n_temporal/npts)};
other_str = {'Extra-temporal: N (%)',fprintf('%d (%1.1f%%)',n_other,n_other/npts)};
