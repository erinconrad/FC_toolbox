...
T = table(vec_soz,vec_pt_idx,vec_rate_sleep,vec_rate_wake,...
    vec_rl_sleep,vec_rl_wake,vec_rate_all,vec_rl_all);
...
if include_rl && include_sw
    glme = fitglme(T_train,...
        'vec_soz ~ vec_rate_sleep + vec_rate_wake + vec_rl_sleep + vec_rl_wake + (1|vec_pt_idx)',...
        'Distribution','Poisson','Link','log');
elseif include_rl && ~include_sw
    glme = fitglme(T_train,...
        'vec_soz ~ vec_rate_all + vec_rl_all + (1|vec_pt_idx)',...
        'Distribution','Poisson','Link','log');
elseif ~include_rl && ~include_sw && dome
    glme = fitglme(T_train,...
        'vec_soz ~ vec_rate_all + (1|vec_pt_idx)',...
        'Distribution','Poisson','Link','log');
elseif ~include_rl && ~include_sw && ~dome
    glme = fitglm(T_train,...
        'vec_soz ~ vec_rate_all',...
        'Distribution','Poisson','Link','log');
elseif ~include_rl && include_sw && ~dome
    glme = fitglm(T_train,...
